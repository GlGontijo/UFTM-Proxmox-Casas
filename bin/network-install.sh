...
rm -f "$DNAT_UP" "$DNAT_DOWN" \
      ...
      /etc/network/if-post-down.d/uftm-dnat-openvpn
...
if [[ "${UFTM_OPNSENSE:-n}" =~ ^[SsYy] ]]; then
  ...
  [[ -n "${UFTM_CONSOLE_BRIDGE:-}" ]] && ENTRIES+=("$UFTM_CONSOLE_BRIDGE 0.0.0.0/0")
  ...
  {
    echo '#!/bin/bash'
    echo '# Gerado por network-install.sh -- DNAT 443 -> OPNsense'
    gen_entries
    cat <<'EOF'
for e in "${ENTRIES[@]}"; do
  read -r i src <<<"$e"
  [ "$IFACE" = "$i" ] || continue
  RULE=(PREROUTING -i "$i" -s "$src" -p tcp --dport 443 -j DNAT --to-destination "$OPN_IP:443")
...
EOF
  ...
  {
    echo '#!/bin/bash'
    echo '# Gerado por network-install.sh -- remove DNAT 443 -> OPNsense'
    gen_entries
    cat <<'EOF'
for e in "${ENTRIES[@]}"; do
  read -r i src <<<"$e"
...
EOF
  ...
  msg_ok "Hooks DNAT 443 -> OPNsense instalados (WAN: $WAN_IF${UFTM_CONSOLE_BRIDGE:+, console: $UFTM_CONSOLE_BRIDGE})"
fi
